...
returns risk estimates and confidence intervals. Both gamma-based and
delta-method confidence intervals are supported through the `ci_method`
argument. The package also supports the piecewise mid-age group
joinpoint (PMAJ) model used in recent DevCan implementations.
...
which is convenient for pipe-based workflows:
...
For multiple registry strata,
[`calc_ltr_df()`](https://github.com/gigu003/ltRISK/reference/calc_ltr_df.md)
can calculate risks by group:
...
By default,
[`calc_ltr()`](https://github.com/gigu003/ltRISK/reference/calc_ltr.md)
returns every valid combination of `age_start` and `age_end`. Use
`age_combine = "pairwise"` for positional, mutually exclusive ranges
such as 0–40, 40–50, and 50 years to the end of life.
...
verification. For many independent registry strata,
[`calc_ltr_df()`](https://github.com/gigu003/ltRISK/reference/calc_ltr_df.md)
...
`cache = "none"` in
[`calc_ltr_df()`](https://github.com/gigu003/ltRISK/reference/calc_ltr_df.md).
This discards memoised entries after each group, including on reused
PSOCK workers, and reduces peak cache growth.
...
[`format_risk_ci()`](https://github.com/gigu003/ltRISK/reference/format_risk_ci.md)
combines estimates and confidence limits for tables while retaining the
precision selected in
[`calc_ltr()`](https://github.com/gigu003/ltRISK/reference/calc_ltr.md).
With `return_variance = TRUE`, grouped results can also be passed to
...
The same
[`calc_ltr()`](https://github.com/gigu003/ltRISK/reference/calc_ltr.md)
interface can be used with the Wun and AMP methods by changing
`risk_func`.
...
Traditional cumulative risk can be calculated directly with
[`calc_ltr()`](https://github.com/gigu003/ltRISK/reference/calc_ltr.md)
...
workflow. Unlike DevCan, Wun, or AMP, this approach does not account for
competing mortality.
